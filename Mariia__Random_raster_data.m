...
raster_data_path = [BASE_PATH 'NDT_self-generated-data\Raster_data'];
...
%     plot(sum(raster_data),'Color',[0,1,0]);
    ...
    folder_of_file{1} = [raster_data_path filesep];
    ...
    fig_name = [folder_of_file{1}, 'random_raster_data_neuron_', num2str(num_of_repetitions(:, n)), '_with_response.png']; %create path and fig name
    saveas(gcf, fig_name); % save the figure 

         
    % create raster_site_info 
    raster_site_info.recording_channel = n;
    raster_site_info.unit = 'with_response';
    raster_site_info.alignment_event_time = 501; 
    
    % save file
    name = ['random_raster_data_neuron_' num2str(num_of_repetitions(:, n)) '_with_response.mat'];
...
%   plot(1:length(frequency_raster_data),frequency_raster_data,'r')
    ...
    %date_time_now = datestr(now,'dd-mm-yy_HH-MM-FFF'); %info about real time (text representing dates and times)
    fig_name = [folder_of_file{1}, 'random_raster_data_neuron_', num2str(num_of_repetitions(:, n)), '_without_response.png']; %create path and fig name
    saveas(gcf, fig_name); % save the figure 


    % create raster_site_info 
    raster_site_info.recording_channel = n;
    raster_site_info.unit = 'without_response';
    raster_site_info.alignment_event_time = 501; 
    
    % save file
    name = ['random_raster_data_neuron_' num2str(num_of_repetitions(:, n)) '_without_response.mat'];
